...
TEST_CASE(test_init_bounds) {
    ...
    CHECK(cfgpack_init(&ctx, &schema, values, 1, defaults,
                       str_pool, sizeof(str_pool), str_offsets, 0) == CFGPACK_ERR_BOUNDS);
...
TEST_CASE(test_pagein_zero_len) {
    ...
    CHECK(cfgpack_init(&ctx, &schema, values, 1, defaults,
                       str_pool, sizeof(str_pool), str_offsets, 0) == CFGPACK_OK);
...
TEST_CASE(test_pageout_too_large) {
    ...
    CHECK(cfgpack_set_u8(&ctx, 1, 1) == CFGPACK_OK);
...
TEST_CASE(test_type_mismatch_and_len) {
    ...
    LOG("Initializing context");
    CHECK(cfgpack_init(&ctx, &schema, values, 2, defaults,
                       str_pool, sizeof(str_pool), str_offsets, 1) == CFGPACK_OK);
    LOG("Context initialized successfully");
    ...
    CHECK(cfgpack_set_str(&ctx, 2, long_str) == CFGPACK_ERR_STR_TOO_LONG);
...
TEST_CASE(test_presence_reset) {
    ...
    CHECK(cfgpack_set_u8(&ctx, 1, 1) == CFGPACK_OK);
    CHECK(cfgpack_set_u8(&ctx, 2, 2) == CFGPACK_OK);
...
TEST_CASE(test_pageout_file_roundtrip_and_io) {
    ...
    CHECK(cfgpack_set_u8(&ctx, 1, 9) == CFGPACK_OK);
    LOG("Value set successfully");
...
TEST_CASE(test_pageout_pagein_all_types) {
    ...
    cfgpack_value_t out;
    /* 2 string entries: index 11 (str), index 12 (fstr) */
    char str_pool[256];
    ...
    LOG("Initializing context");
    CHECK(cfgpack_init(&ctx, &schema, values, 12, defaults,
                       str_pool, sizeof(str_pool), str_offsets, 2) == CFGPACK_OK);
    LOG("Context initialized successfully");
    ...
    CHECK(cfgpack_set_u8(&ctx, 1, 1) == CFGPACK_OK);
    LOG("  [1] u8 = 1");
    CHECK(cfgpack_set_u16(&ctx, 2, 2) == CFGPACK_OK);
    LOG("  [2] u16 = 2");
    CHECK(cfgpack_set_u32(&ctx, 3, 3) == CFGPACK_OK);
    LOG("  [3] u32 = 3");
    CHECK(cfgpack_set_u64(&ctx, 4, 4) == CFGPACK_OK);
    LOG("  [4] u64 = 4");
    CHECK(cfgpack_set_i8(&ctx, 5, -1) == CFGPACK_OK);
    LOG("  [5] i8 = -1");
    CHECK(cfgpack_set_i16(&ctx, 6, -2) == CFGPACK_OK);
    LOG("  [6] i16 = -2");
    CHECK(cfgpack_set_i32(&ctx, 7, -3) == CFGPACK_OK);
    LOG("  [7] i32 = -3");
    CHECK(cfgpack_set_i64(&ctx, 8, -4) == CFGPACK_OK);
    LOG("  [8] i64 = -4");
    CHECK(cfgpack_set_f32(&ctx, 9, 1.25f) == CFGPACK_OK);
    LOG("  [9] f32 = 1.25");
    CHECK(cfgpack_set_f64(&ctx, 10, 2.5) == CFGPACK_OK);
    LOG("  [10] f64 = 2.5");
    CHECK(cfgpack_set_str(&ctx, 11, "foo") == CFGPACK_OK);
    LOG("  [11] str = \"foo\"");
    CHECK(cfgpack_set_fstr(&ctx, 12, "bar") == CFGPACK_OK);
    LOG("  [12] fstr = \"bar\"");
    ...
    const char *str_out;
    uint16_t str_len;
    CHECK(cfgpack_get_str(&ctx, 11, &str_out, &str_len) == CFGPACK_OK && str_len == 3 && strncmp(str_out, "foo", 3) == 0);
    LOG("  [11] str = \"%s\" (ok)", str_out);

    const char *fstr_out;
    uint8_t fstr_len;
    CHECK(cfgpack_get_fstr(&ctx, 12, &fstr_out, &fstr_len) == CFGPACK_OK && fstr_len == 3 && strncmp(fstr_out, "bar", 3) == 0);
...
TEST_CASE(test_remap_basic) {
    ...
    CHECK(cfgpack_init(&old_ctx, &old_schema, old_values, 1, old_defaults,
                       old_str_pool, sizeof(old_str_pool), old_str_offsets, 0) == CFGPACK_OK);
    CHECK(cfgpack_init(&new_ctx, &new_schema, new_values, 1, new_defaults,
                       new_str_pool, sizeof(new_str_pool), new_str_offsets, 0) == CFGPACK_OK);
...
TEST_CASE(test_remap_type_widening) {
    ...
    CHECK(cfgpack_set_u8(&old_ctx, 1, 200) == CFGPACK_OK);
    LOG("Value set successfully");
...
TEST_CASE(test_remap_type_narrowing_rejected) {
    ...
    CHECK(cfgpack_set_u16(&old_ctx, 1, 1000) == CFGPACK_OK);
    LOG("Value set successfully");
