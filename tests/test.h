...
#define LOG(fmt, ...) printf(COLOR_DIM "    " fmt COLOR_RESET "\n", ##__VA_ARGS__)

/* Log a section header within a test */
#define LOG_SECTION(title) printf(COLOR_CYAN "    --- " title " ---" COLOR_RESET "\n")
...
#define LOG_FAT_VALUE(label, val)                                                                                      \
    do {                                                                                                               \
        switch ((val).type) {                                                                                          \
        case CFGPACK_TYPE_U8: LOG("%s: u8 = %" PRIu64, label, (val).v.u64); break;                                     \
        case CFGPACK_TYPE_U16: LOG("%s: u16 = %" PRIu64, label, (val).v.u64); break;                                   \
        ...
        case CFGPACK_TYPE_F32: LOG("%s: f32 = %f", label, (double)(val).v.f32); break;                                 \
        case CFGPACK_TYPE_F64: LOG("%s: f64 = %f", label, (val).v.f64); break;                                         \
        case CFGPACK_TYPE_STR: LOG("%s: str[%u] = \"%s\"", label, (val).v.str.len, (val).v.str.data); break;           \
        case CFGPACK_TYPE_FSTR: LOG("%s: fstr[%u] = \"%s\"", label, (val).v.fstr.len, (val).v.fstr.data); break;       \
        default: LOG("%s: unknown type %d", label, (val).type); break;                                                 \
        }                                                                                                              \
    } while (0)
