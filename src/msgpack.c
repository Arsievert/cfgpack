...
cfgpack_err_t cfgpack_msgpack_encode_uint64(cfgpack_buf_t *buf, uint64_t v) {
    ...
    if (v <= 0x7fu) {
        ...
        tmp[n++] = 0xcc; tmp[n++] = (uint8_t)v;
    } else if (v <= 0xffffu) {
        tmp[n++] = 0xcd; tmp[n++] = (uint8_t)(v >> 8); tmp[n++] = (uint8_t)v;
        ...
        tmp[n++] = 0xcf;
        for (int i = 7; i >= 0; --i) tmp[n++] = (uint8_t)(v >> (8 * i));
    }
...
cfgpack_err_t cfgpack_msgpack_encode_int64(cfgpack_buf_t *buf, int64_t v) {
    ...
    if (v >= -32) {
        ...
        tmp[n++] = 0xd0; tmp[n++] = (uint8_t)v;
    } else if (v >= -32768) {
        tmp[n++] = 0xd1; tmp[n++] = (uint8_t)(v >> 8); tmp[n++] = (uint8_t)v;
    } else if (v >= INT32_MIN) {
        tmp[n++] = 0xd2;
        tmp[n++] = (uint8_t)(v >> 24);
        tmp[n++] = (uint8_t)(v >> 16);
...
cfgpack_err_t cfgpack_msgpack_encode_f32(cfgpack_buf_t *buf, float v) {
    uint32_t u;
    memcpy(&u, &v, sizeof(u));
    uint8_t tmp[5] = {0xca, (uint8_t)(u >> 24), (uint8_t)(u >> 16), (uint8_t)(u >> 8), (uint8_t)u};
...
cfgpack_err_t cfgpack_msgpack_encode_f64(cfgpack_buf_t *buf, double v) {
    ...
    uint8_t tmp[9];
    tmp[0] = 0xcb;
    for (int i = 0; i < 8; ++i) tmp[1 + i] = (uint8_t)(u >> (56 - 8 * i));
    return cfgpack_buf_append(buf, tmp, sizeof(tmp));
}
...
cfgpack_err_t cfgpack_msgpack_encode_str(cfgpack_buf_t *buf, const char *s, size_t len) {
    ...
    if (len <= 31) {
        ...
        hdr[hlen++] = 0xd9; hdr[hlen++] = (uint8_t)len;
    } else {
        hdr[hlen++] = 0xda; hdr[hlen++] = (uint8_t)(len >> 8); hdr[hlen++] = (uint8_t)len;
    }
    if (cfgpack_buf_append(buf, hdr, hlen) != CFGPACK_OK) return CFGPACK_ERR_ENCODE;
...
cfgpack_err_t cfgpack_msgpack_encode_map_header(cfgpack_buf_t *buf, uint32_t count) {
    ...
    if (count <= 15) {
        ...
        tmp[n++] = 0xde; tmp[n++] = (uint8_t)(count >> 8); tmp[n++] = (uint8_t)count;
    }
    return cfgpack_buf_append(buf, tmp, n);
}
...
cfgpack_err_t cfgpack_msgpack_decode_map_header(cfgpack_reader_t *r, uint32_t *count) {
    uint8_t b;
    if (read_bytes(r, &b, 1)) return CFGPACK_ERR_DECODE;
    if ((b & 0xf0u) == 0x80u) {
    ...
    if (b == 0xde) {
        uint8_t bytes[2];
        if (read_bytes(r, bytes, 2)) return CFGPACK_ERR_DECODE;
...
cfgpack_err_t cfgpack_msgpack_decode_uint64(cfgpack_reader_t *r, uint64_t *out) {
    uint8_t b;
    if (read_bytes(r, &b, 1)) return CFGPACK_ERR_DECODE;
    if (b <= 0x7f) { *out = b; return CFGPACK_OK; }
    if (b == 0xcc) { uint8_t v; if (read_bytes(r, &v, 1)) return CFGPACK_ERR_DECODE; *out = v; return CFGPACK_OK; }
    if (b == 0xcd) { uint8_t v[2]; if (read_bytes(r, v, 2)) return CFGPACK_ERR_DECODE; *out = ((uint16_t)v[0] << 8) | v[1]; return CFGPACK_OK; }
    if (b == 0xce) { uint8_t v[4]; if (read_bytes(r, v, 4)) return CFGPACK_ERR_DECODE; *out = ((uint32_t)v[0] << 24) | ((uint32_t)v[1] << 16) | ((uint32_t)v[2] << 8) | v[3]; return CFGPACK_OK; }
    if (b == 0xcf) { uint8_t v[8]; if (read_bytes(r, v, 8)) return CFGPACK_ERR_DECODE; uint64_t res = 0; for (int i = 0; i < 8; ++i) res = (res << 8) | v[i]; *out = res; return CFGPACK_OK; }
    return CFGPACK_ERR_DECODE;
}
...
cfgpack_err_t cfgpack_msgpack_decode_int64(cfgpack_reader_t *r, int64_t *out) {
    uint8_t b;
    if (read_bytes(r, &b, 1)) return CFGPACK_ERR_DECODE;
    if (b <= 0x7f) { *out = (int64_t)b; return CFGPACK_OK; }
    if (b >= 0xe0) { *out = (int8_t)b; return CFGPACK_OK; }
    if (b == 0xd0) { int8_t v; if (read_bytes(r, &v, 1)) return CFGPACK_ERR_DECODE; *out = v; return CFGPACK_OK; }
    if (b == 0xd1) { int16_t v; if (read_bytes(r, &v, 2)) return CFGPACK_ERR_DECODE; v = (int16_t)((uint16_t)(uint8_t)(v >> 8) | ((uint16_t)(uint8_t)v << 8)); *out = v; return CFGPACK_OK; }
    if (b == 0xd2) { uint8_t v[4]; if (read_bytes(r, v, 4)) return CFGPACK_ERR_DECODE; int32_t res = (int32_t)((uint32_t)v[0] << 24 | (uint32_t)v[1] << 16 | (uint32_t)v[2] << 8 | v[3]); *out = res; return CFGPACK_OK; }
    if (b == 0xd3) { uint8_t v[8]; if (read_bytes(r, v, 8)) return CFGPACK_ERR_DECODE; int64_t res = 0; for (int i = 0; i < 8; ++i) res = (res << 8) | v[i]; *out = res; return CFGPACK_OK; }
    return CFGPACK_ERR_DECODE;
}
...
cfgpack_err_t cfgpack_msgpack_decode_f32(cfgpack_reader_t *r, float *out) {
    ...
    uint8_t v[4];
    if (read_bytes(r, &b, 1)) return CFGPACK_ERR_DECODE;
    if (b != 0xca) return CFGPACK_ERR_DECODE;
    if (read_bytes(r, v, 4)) return CFGPACK_ERR_DECODE;
    uint32_t u = ((uint32_t)v[0] << 24) | ((uint32_t)v[1] << 16) | ((uint32_t)v[2] << 8) | v[3];
...
cfgpack_err_t cfgpack_msgpack_decode_f64(cfgpack_reader_t *r, double *out) {
    ...
    uint8_t v[8];
    if (read_bytes(r, &b, 1)) return CFGPACK_ERR_DECODE;
    if (b != 0xcb) return CFGPACK_ERR_DECODE;
    if (read_bytes(r, v, 8)) return CFGPACK_ERR_DECODE;
    uint64_t u = 0; for (int i = 0; i < 8; ++i) u = (u << 8) | v[i];
    memcpy(out, &u, sizeof(u));
    return CFGPACK_OK;
}
...
cfgpack_err_t cfgpack_msgpack_decode_str(cfgpack_reader_t *r, const uint8_t **ptr, uint32_t *len) {
    ...
    if ((b & 0xe0u) == 0xa0u) {
        *len = (uint32_t)(b & 0x1f);
        ...
        uint8_t l; if (read_bytes(r, &l, 1)) return CFGPACK_ERR_DECODE; *len = l;
    } else if (b == 0xda) {
        uint8_t v[2]; if (read_bytes(r, v, 2)) return CFGPACK_ERR_DECODE; *len = ((uint32_t)v[0] << 8) | v[1];
    } else {
        return CFGPACK_ERR_DECODE;
    }
